...
- 401 + Hospital Logo is empty: เอกสารระบุว่าไม่มีรูปที่ CMS Alert แต่คู่มือ Admin ที่ได้รับไม่มีขั้นตอนอัปโหลดรูป หาก CMS ไม่มีเมนู ให้ติดต่อผู้ดูแล MOPH เพื่อตั้งโลโก้หน่วยบริการ
...
## โลโก้โรงพยาบาลบนเว็บ

เว็บใช้ไฟล์ `public/hospital-logo.png` สำหรับหน้าปฏิทิน หน้าเข้าสู่ระบบ หน้าผู้ดูแล และไอคอนเว็บ เปิดไฟล์ต้นฉบับได้ที่ `/hospital-logo.png`. รูปนี้เป็นส่วนของเว็บเรา ไม่ได้ตั้งรูปหน่วยบริการในระบบ MOPH และไม่แก้ `Hospital Logo is empty` โดยอัตโนมัติ

เอกสาร Free Form 3.1 และ Postman ที่ได้รับไม่มีพารามิเตอร์ `logo` หรือ `logo_url` จึงไม่เพิ่มฟิลด์ที่ไม่ได้ระบุใน API. หากผู้ดูแล MOPH ขอ URL รูป ต้องใช้ URL สาธารณะผ่าน HTTPS หลังเผยแพร่เว็บ; `localhost` หรือ path ไฟล์บนเครื่องไม่ใช่ URL ที่ MOPH เข้าถึงได้
...
ทดสอบ classification, timeout ผ่าน fake transport, scheduler deduplication และ DRY_RUN ที่ไม่เรียก provider. การทดสอบอัตโนมัติไม่ส่งข้อความจริง และไม่มี credential จริงใน repository. ก่อนใช้งานจริงให้ทดสอบกับผู้รับที่ตรวจรับแล้วหนึ่งคน ตรวจรูปแบบ payload/สิทธิ์หน่วยบริการ/LINE OA กับ MOPH และยืนยันการรับข้อความปลายทางด้วยตนเอง
