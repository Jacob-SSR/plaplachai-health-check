...
ทันตกรรม แผนไทย กายภาพ และตรวจเลือด เป็นบริการกลางถาวร ใช้รหัสบริการเดียวกันทุกปี. หลัง `npm run db:migrate` ระบบเตรียมบริการทั้ง 4 ให้ปีที่เปิดอยู่ และเมื่อเพิ่มปีใหม่จะพร้อมใช้โดยอัตโนมัติ ไม่ต้องสร้างแผนหรือบริการซ้ำ. ทะเบียน วันนัด และผลการเข้ารับบริการยังแยกตามปี; ข้อมูลแผนและนัดเดิมคงอยู่

Admin → แจ้งเตือน → เลือกผู้รับ → **ส่งข้อความทดสอบ** ส่งทันทีโดยไม่ต้องมีนัดหรือรอ worker. ต้องตั้ง Client_ID/Secret, MOPH_LIVE_ENABLED=true และเปิดโหมด LIVE พร้อมผู้รับที่ตรวจสอบเลขบัตรและเปิดรับแล้ว. แสดงผลรับคำขอ/ส่งไม่ผ่าน/ยังยืนยันไม่ได้ ตรวจข้อความจริงใน LINE หมอพร้อมของผู้รับด้วย อ่าน [วิธีทดสอบ](docs/MOPH-ALERT.md)

## เริ่มบันทึกงานในเว็บ

1. ข้อมูลตั้งต้น: นำเข้าบุคลากรทั้งชุดเพื่อเพิ่มกลุ่มงาน/หน่วยงาน/ตำแหน่ง แล้วเพิ่มปีงบประมาณ บริการประจำทั้ง 4 พร้อมใช้ให้อัตโนมัติ
2. บุคลากร: ตรวจรายชื่อและหน่วยงานที่นำเข้า เพิ่มหรือแก้เฉพาะข้อมูลใหม่ในอนาคต ไม่จำเป็นต้องกรอกเลขบัตรเพื่อจัดนัด
3. ผู้มีสิทธิ์ประจำปี: เลือกปีแล้วกดเพิ่มผู้มีสิทธิ์ เลือก “เพิ่มบุคลากรที่เปิดใช้งานทั้งหมด” เพื่อเข้าทั้งแผนในครั้งเดียว เลือกรายการตรวจ/จำนวนรอบ/วันอ้างอิงที่อยู่ในปีและมีประวัติตำแหน่งครอบคลุม
